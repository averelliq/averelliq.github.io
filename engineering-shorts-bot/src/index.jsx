...
// Five non-jet topics retain topic-specific schematic motion graphics.
// These are explicitly illustrations, not photographic or verified CAD simulations.
...
const Scene=({scene,topic,number,total})=>{
  ...
  // Approximate visual captions: word-aligned transcript is a future improvement.
  ...
  const visualZoom=interpolate(frame,[0,duration],[1,1.018],{extrapolateRight:'clamp'});
  // No opacity fades: the previous V2 preview had an all-black frame at every scene boundary.
  return <AbsoluteFill style={{color:palette.white,fontFamily:'Arial, sans-serif',background:'radial-gradient(ellipse at 50% 39%,#174462 0%,#07182a 67%,#040d17 100%)'}}>
    ...
    <div style={{position:'absolute',top:315,left:40,right:40,height:1090,border:'3px solid #285872',borderRadius:44,overflow:'hidden',background:'linear-gradient(160deg,#123249,#061928)',boxShadow:'0 12px 65px #01070d',transform:`scale(${visualZoom})`}}>
    ...
    <div style={{position:'absolute',top:1444,left:68,right:68,minHeight:166,display:'flex',alignItems:'center',justifyContent:'center',textAlign:'center',fontSize:54,fontWeight:900,lineHeight:1.17,textShadow:'0 4px 17px #000'}}>{caption}</div>
